...
import com.example.test2.databinding.ActivitySignUpInBinding
...
class SigninUPAct : AppCompatActivity() {
    ...
    override fun onCreate(s: Bundle?) {
        ...
        signState = intent.getStringExtra(Constance.SING_STATE)!!
        if(signState == Constance.SING_IN_STATE){
    ...
    fun onClickDone(view: View){
        ...
        if(signState == Constance.SING_UP_STATE){
            ...
            if(bindingClass.imAvatar2.isVisible)intent.putExtra(Constance.AVATAR_ID, R.drawable.andrey)
            setResult(RESULT_OK, intent)
            finish()

        } else if(signState == Constance.SING_IN_STATE){
